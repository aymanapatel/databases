...
Since it is hashed with mod; there will be collisions. Multi-level hash functions can help woth reducing collisions.

Cassandra (and dynamo DB) uses this in Consistant Hashing using SSTables.


Ayman NOTE: Merkle tree, SSTable and Boom filter has some correlation. Do not have insight on the relation though.
